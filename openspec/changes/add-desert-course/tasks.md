...
- [x] **Task 8** — Update `holes.ts` to re-export `ALL_HOLES` as `PRESET_HOLES` → `CHG-COURSE-008` → `holes.ts` ✓
- [x] **Task 9** — Verify `getRandomHole` works with combined array → `CHG-COURSE-009` → `holeSelection.ts` ✓
